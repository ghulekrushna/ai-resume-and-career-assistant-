...
const DepthCarousel = ({
  ...
  const onCardClick = useCallback(
    index => {
      ...
      if (active === index && onItemSelect) {
        onItemSelect(data[index], index);
      } else {
        setFocus(index, true);
      }
    },
    [active, data, onItemSelect, setFocus]
  );
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            <img className="depth-carousel__img" src={item.image} alt={item.alt || item.name || ''} draggable={false} />
            <span
            ...
            />
            {item.name && (
              <div className="depth-carousel__badge">
                <span className="depth-carousel__badge-text">{item.name}</span>
                {active === i && (
                  <span className="depth-carousel__badge-action">Click to Preview &rarr;</span>
                )}
              </div>
            )}
          </div>
